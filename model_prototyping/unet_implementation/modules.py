...
from model_prototyping.unet_implementation.metrics import (
    show_sample,
    plot_weighted_running_average_loss,
)
...
from structured_data_utils.structured_data_interfacing import (
    tensor_and_offset_from_geotiff,
)
...
class SimpleUnet(nn.Module):
    class Block(nn.Module):
        def __init__(
            self, in_ch: int, out_ch: int, groups: int = 8, block_config: dict = None
        ):
            super().__init__()
    ...
    def __init__(
        ...
        self.bottleneck = self.Block(
            features[-1], features[-1] * 2, block_config=block_config
        )
...
def update_lr(
    current_lr: float, lr_decay: float, lr_decay_function: str, total_steps: int
) -> float:
...
def train_model(
    ...
    viz_steps: int = 1,
    pos_bias: float = 1.0,
    features: List[int] = [8, 16, 32],
    lr=1e-3,
    lr_decay: float = 1.00003,
    lr_decay_function: str = "exp",
    block_config: dict = None,
) -> SimpleUnet:
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = SimpleUnet(
        in_channels=1, out_channels=1, features=features, block_config=block_config
    ).to(device)
    ...
    fixed_data = all_data[fixed_indices].to(device).float()  # (N, 1, H, W)
    fixed_labels = all_labels[fixed_indices].to(device).float()  # (N, 1, H, W)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, (this_data, labels) in enumerate(
            data.segmented_data_with_labels.get_hacky_fold_iterable()
        ):
            # Make (B,C,H,W) with B=C=1
            this_data = this_data.unsqueeze(0).to(device).float()
            labels = labels.unsqueeze(0).to(device).float()
    ...
    plot_weighted_running_average_loss(
        ...
        fixed_sample_losses=fixed_sample_losses,
    )
...
def test_model_on_model_data(model: SimpleUnet, test_data: ModelData):
    ...
    for (
        data,
        empty_labels,
    ) in test_data.segmented_data_with_labels.get_hacky_fold_iterable():
        ...
        print(
            f"Logits statistics: min={logits_min:.6f}, max={logits_max:.6f}, mean={logits_mean:.6f}"
        )
...
def test_model_visual(model: SimpleUnet, test_data_file: str = None):
    ...
    print(
        "If you got the warning about an empty tensor, that's expected for the test set."
    )
